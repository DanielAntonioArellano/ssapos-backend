...
export class TicketsService {
  ...
  private separator() { return '='.repeat(this.LINE_WIDTH); }
  private separatorThin() { return '-'.repeat(this.LINE_WIDTH); }

  private center(text: string) {
    if (!text) return '';
    if (text.length >= this.LINE_WIDTH) return text.slice(0, this.LINE_WIDTH);
  ...
  private alignLeftRight(left: string, right: string) {
    ...
    if (left.length + right.length >= this.LINE_WIDTH) {
      return left.slice(0, this.LINE_WIDTH - right.length - 1) + ' ' + right;
    }
  ...
  async getSaleTicket(restaurantId: number, saleId: number) {
    return this.prisma.$transaction(async (tx) => {
      const sale = await tx.sale.findFirst({
        ...
        include: { user: true, items: { include: { product: true } }, restaurant: true },
      });
      if (!sale) throw new NotFoundException('Venta no encontrada');

      const folio = sale.restaurant.folioVenta;
      await tx.restaurant.update({ where: { id: restaurantId }, data: { folioVenta: { increment: 1 } } });

      const subtotalReal = sale.items.reduce((s, item) => s + item.priceUnit * item.quantity, 0);
      ...
      return {
        ...
        lines: [
          ...
          ...(hayDescuento ? [this.alignLeftRight('Descuento', `-$${descuento.toFixed(2)}`)] : []),
          this.alignLeftRight('Metodo de pago', sale.payment ?? ''),
          ...(hayPropina ? [this.alignLeftRight('Propina (tarjeta)', `$${tip.toFixed(2)}`)] : []),
          this.separator(),
          this.alignLeftRight('TOTAL', `$${sale.total.toFixed(2)}`),
          ...(hayPropina ? [this.alignLeftRight('TOTAL + PROPINA', `$${totalConPropina.toFixed(2)}`)] : []),
          this.separator(),
  ...
  async getOrderTicket(restaurantId: number, orderId: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.restaurant.update({ where: { id: restaurantId }, data: { folioOrden: { increment: 1 } } });
  ...
  buildCuentaParcialLines(data: {
    ...
    const hayDescuento = (data.descuento ?? 0) > 0.01;
    const itemLines = data.items.map((item) =>
      this.alignLeftRight(`  ${item.quantity}x ${item.name}`, `$${item.subtotal.toFixed(2)}`),
    );

    return [
      this.separator(),
      this.center(`*** CUENTA ${data.numeroCuenta} ***`),
      this.separator(),
      `Fecha: ${this.formatDate(new Date())}`,
      this.separatorThin(),
      this.center('PRODUCTOS'),
      ...
      ...(hayDescuento
        ? [
            this.alignLeftRight('Subtotal', `$${data.subtotal.toFixed(2)}`),
            this.alignLeftRight('Descuento', `-$${(data.descuento ?? 0).toFixed(2)}`),
          ]
        : []),
      this.alignLeftRight('Metodo de pago', data.payment),
      this.separator(),
  ...
  async getCorteTicket(restaurantId: number, cajaId: number) {
    return this.prisma.$transaction(async (tx) => {
      const caja = await tx.caja.findFirst({
        ...
        include: { user: true, restaurant: true, ventas: true, gastos: true, movimientos: true },
      });
      if (!caja) throw new NotFoundException('Caja no encontrada');

      const folio = caja.restaurant.folioCorte;
      await tx.restaurant.update({ where: { id: restaurantId }, data: { folioCorte: { increment: 1 } } });
  ...
  private buildCorteTicket(caja: any, folio: number) {
    const ventasEfectivo = caja.ventas.filter((v) => (v.payment ?? '').toLowerCase() === 'efectivo').reduce((s, v) => s + v.total, 0);
    const ventasTarjeta  = caja.ventas.filter((v) => (v.payment ?? '').toLowerCase() === 'tarjeta').reduce((s, v) => s + v.total, 0);
    ...
    const entradasLines = entradas.length > 0
      ? [this.separatorThin(), this.center('ENTRADAS'), this.separatorThin(),
         ...entradas.map((m) => this.alignLeftRight(`  ${m.descripcion ?? 'Sin concepto'}`, `$${m.monto.toFixed(2)}`)),
    ...
    const gastosLines = gastos.length > 0
      ? [this.separatorThin(), this.center('GASTOS'), this.separatorThin(),
         ...gastos.map((g) => this.alignLeftRight(`  ${g.concepto ?? 'Sin concepto'}`, `-$${g.monto.toFixed(2)}`)),
    ...
    const propinasLines = hayPropinas
      ? [this.separatorThin(), this.center('PROPINAS'), this.separatorThin(),
    ...
    return {
      ...
      lines: [
        ...
        ...(hayPropinas ? [this.alignLeftRight('PROPINAS DEL TURNO', `$${propinasTotal.toFixed(2)}`)] : []),
        this.separator(),
